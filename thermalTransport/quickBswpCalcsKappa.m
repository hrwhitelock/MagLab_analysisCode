...
function dataOut = quickBswpCalcsKappa(fname, geometricFactor) %, hotFit, coldFit,bathFit)
    % data = load(fname); 
    ...
    % % redo the cx fits --> comment this out for quick analysis
...
%         data.dT(offidx(j):offidx(j+1)-1) = hot-cold; 
        ...
        dataOut.avgColdOff(j) = mean(data.coldTemp(offidx(j)+skipPts:onidx(j)));
