% function to antisymmetrize data
% hope
% Oct 24, 2024

function [antiSymX, antiSymY] = antiSymData(x,y, negXlim, posXlim)
...
%     plot(interpX, interpY)
    ...
    for posidx = (numpoints-1)/2+1:numpoints
        ...
        antiSymY(j) = (interpY(posidx)-interpY(negidx))/2;
        antiSymX(j) = interpX(posidx);
        j = j+1; 
    end
